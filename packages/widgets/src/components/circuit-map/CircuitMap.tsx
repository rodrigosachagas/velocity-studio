...
export function CircuitMap({
  ...
  const gpsFrames = useMemo(() => {
    ...
    if (valid.length <= 800) return valid
    const step = Math.ceil(valid.length / 800)
  ...
  // For circuit style: project only the best complete lap (tight single-lap outline).
  // Uses the same projParams as the full track so the car dot stays aligned.
  const circuitLapProjected = useMemo(() => {
    if (trackStyle !== "circuit" || !startFinishLine || !projParams || gpsFrames.length < 2) return null
    const session = computeLaps(gpsFrames, startFinishLine)
    if (session.laps.length === 0) return null
    const best = session.laps.reduce((b, l) => l.lapTimeMs < b.lapTimeMs ? l : b, session.laps[0]!)
    const lapFrames = gpsFrames.filter(
      (f) => f.timestamp >= best.startMs && f.timestamp <= best.endMs,
    )
    if (lapFrames.length < 4) return null
    const maxSpeed = Math.max(...gpsFrames.map((f) => f.speed ?? 0), 1)
    return lapFrames.map((f) => projectGPS(f, projParams, maxSpeed))
  }, [trackStyle, startFinishLine, gpsFrames, projParams])
  ...
  const trackPath = useMemo(() => {
    ...
    return (
      <g>
        ...
        {/* Label: "BEST LAP" when using single-lap trace */}
        {circuitLapProjected && (
          <text
            x={pad} y={height - pad}
